        ...
        for key ,value in data.items():
            try:
                if data[key]['sourceType']=='video':
                    ...
                    write_json(data[key],'video',key+'.json')
                    convert('video/'+key+'.json','video/'+key+'.xbs',0)
                elif data[key]['sourceType']=='comic':
                    comic_list[key]=data[key]
                    write_json(data[key],'comic',key+'.json')
                    convert('comic/'+key+'.json','comic/'+key+'.xbs',0)
                elif data[key]['sourceType']=='audio':
                    audio_list[key]=data[key]
                    write_json(data[key],'audio',key+'.json')
                    convert('audio/'+key+'.json','audio/'+key+'.xbs',0)
                elif data[key]['sourceType']=='text':
                    text_list[key]=data[key]
                    write_json(data[key],'text',key+'.json')
                    convert('text/'+key+'.json','text/'+key+'.xbs',0)
            except KeyError:
                data[key]['sourceType'] = 'text'
                text_list[key]=data[key]
                write_json(data[key],'text',key+'.json')
                convert('text/'+key+'.json','text/'+key+'.xbs',0)
        write_json(video_list,'video', 'videoList.json')
        write_json(comic_list,'comic', 'comicList.json')
        write_json(audio_list, 'audio', 'audioList.json')
        write_json(text_list, 'text','textList.json')
